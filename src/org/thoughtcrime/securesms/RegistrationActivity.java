  ...
  private void initializeBackupDetection() {
    ...
    new AsyncTask<Void, Void, BackupUtil.BackupInfo>() {
      ...
      protected @Nullable BackupUtil.BackupInfo doInBackground(Void... voids) {
        try {
          // JW: changes to detect backups that are on the removable SD card
          BackupUtil.BackupInfo backupInfo = BackupUtil.getLatestBackup();
          if (backupInfo == null) {
            // No backup found in internal storage, check the removable storage.
            TextSecurePreferences.setBackupLocationRemovable(RegistrationActivity.this, true);
            backupInfo = BackupUtil.getLatestBackup();
            if (backupInfo == null) {
              // If backup is found on removable storage, leave the setting to removable storage. Else reset to default (false)
              TextSecurePreferences.setBackupLocationRemovable(RegistrationActivity.this, false);
            }
          }
          return backupInfo;
          // End JW changes
